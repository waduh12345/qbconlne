...
import { useSession as useSessions } from "next-auth/react"; // alias useSessions sesuai permintaan
...
function LmsPageInner() {
  ...
  // ambil session (alias useSessions)
  const { data: session, status: sessionStatus } = useSessions();
  const currentUserId = session?.user?.id ?? null;

  // ambil student object langsung dari session (typed)
  const sessionStudent = (session?.user as { student?: Student | undefined })
    ?.student;
  ...
  // handling loading / unauthorized / not-premium
  if (sessionStatus === "loading") {
    return <PageSkeleton />;
  }

  if (!currentUserId) {
    // tidak ada session — ajak login
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="max-w-lg rounded-2xl border border-sky-100 bg-white p-6 text-center">
          <h2 className="mb-2 text-lg font-semibold">Silakan masuk</h2>
          <p className="mb-4 text-sm text-zinc-600">
            Untuk melihat materi LMS, silakan login terlebih dahulu.
          </p>
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_left,rgba(14,165,233,0.06),transparent_40%),radial-gradient(ellipse_at_bottom_right,rgba(99,102,241,0.06),transparent_40%)]">
      <div className="mx-auto w-full max-w-6xl px-4 py-8">
        ...
        {lmsData?.last_page && lmsData.last_page > 1 && (
          <div className="mt-8 flex items-center justify-center gap-3">
            ...
            <div className="rounded-lg border border-sky-200 bg-white px-3 py-1 text-sm text-sky-700">
              {lmsData.current_page} / {lmsData.last_page}
            </div>
            <Button
              variant="outline"
              className="rounded-xl border-sky-200 text-sky-700"
              disabled={page >= lmsData.last_page}
              onClick={() =>
                setPage((p) => Math.min(lmsData.last_page ?? p, p + 1))
              }
            >
              Berikutnya
            </Button>
